...
uint64 FNV_Hash(const cv::Mat& img, HashCoord* coords, int len, bool use_RGB)
{
	uint64 hash = FNV_OFFSET_BASIS_64;

	size_t coord_count = 0;
	HashCoord* coord = coords;
...
// hash upper, lower, and combined separately 
uint32 FNV_Hash_Combined_32(cv::Mat img, uint32& hash_upper, uint32& hash_lower, const HashCoord* coords, const int len, bool use_RGB)
{
...
uint64 Murmur2_Hash(const cv::Mat& img, HashCoord* coords, int len)
{
	int buflen = len * 3;
	char* buf = new char[buflen];
	int index = 0;
...
uint64 Murmur_Hash_Combined(cv::Mat img, uint64& hash_upper, uint64& hash_lower, const HashCoord* coords, const int len)
{
	return 0;
